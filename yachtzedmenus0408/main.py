...
def check_scores(choice_list, numbers_list, colors_list, suits_list, possible_list, current_score):
    ...
    elif active == 15: #yellows
        current_score = colors_list.count(yellow) * 4
    elif active == 16: #blues
        current_score = colors_list.count(blue) * 4
    elif active == 17: #greens
        current_score = colors_list.count(green) * 4
    elif active == 18: #purples
        current_score = colors_list.count(purple) * 4
    elif active == 20: #spades
        current_score = suits_list.count('s') * 5
    elif active == 21: #hearts
        current_score = suits_list.count('h') * 5
    elif active == 22: #diamonds
        current_score = suits_list.count('d') * 5
    elif active == 23: #clubs
        current_score = suits_list.count('c') * 5

    elif active == 6 or active == 7 or active == 8 or active == 13 or active == 19: #two pair, three of a kind, four of a kind, chance, palette
        if possible_list[active]:
            current_score = sum(numbers_list)
        else:
            current_score = 0
    elif active == 9: #YachtZed
        if possible_list[active]:
            current_score = 50
        else:
            current_score = 0
    elif active == 10: #full house
        if possible_list[active]:
            current_score = 25
        else:
            current_score = 0
    elif active == 11: #small straight
        if possible_list[active]:
            current_score = 30
        else:
            current_score = 0
    elif active == 12: #large straight
        if possible_list[active]:
            current_score = 40
        else:
            current_score = 0
    elif active == 24: #rainbow
        if possible_list[active]:
            current_score = 35
        else:
            current_score = 0
    elif active == 25: #rainbow straight
        if possible_list[active]:
            current_score = 50
        else:
            current_score = 0
    elif active == 26: #color flush
        if possible_list[active]:
            current_score = 45
        else:
            current_score = 0
    elif active == 27: #flush
        if possible_list[active]:
            current_score = 30
        else:
            current_score = 0
    elif active == 28: #straight flush
        if possible_list[active]:
            current_score = 50
        else:
            current_score = 0
    elif active == 29: #rainbow flush
        if possible_list[active]:
            current_score = 45
        else:
            current_score = 0
    elif active == 30: #yacht flush
        if possible_list[active]:
            current_score = 75
        else:
            current_score = 0
    elif active == 31: #rainbow yacht
        if possible_list[active]:
            current_score = 75
        else:
            current_score = 0
    elif active == 32: #yacht double flush
        if possible_list[active]:
            current_score = 100
        else:
            current_score = 0
    elif active == 33: #rainbow yacht flush
        if possible_list[active]:
            current_score = 90
        else:
            current_score = 0
...
def check_possible(possible_list, numbers_list, colors_list, suits_list):
    ...
    if max_count >= 3: #three numbers of a kind
        possible_list[7] = True #three of a kind is true
        if max_count >= 4: #four numbers of a kind
            possible_list[8] = True #four of a kind is true
            possible_list[6] = True #two pair is also true if there are 4 of a kind
            if max_count == 5: #YachtZed!
                possible_list[9] = True #yachtzed is true
                possible_list[10] = True #full house is true for a yachtzed

    if max_count < 3: #reseting if numbers are too small
        possible_list[7] = False
        possible_list[8] = False
        possible_list[9] = False
        possible_list[10] = False

    if max_count == 3: #logic for full house
        possible_list[8] = False
        possible_list[9] = False
        checker = False
        for index in range(len(numbers_list)):
            if numbers_list.count(numbers_list[index]) == 2:
                possible_list[10] = True #full house true
                possible_list[6] = True #two pair also true
                checker = True
        if not checker:
            possible_list[10] = False
